...
class Misc(commands.Cog):
    ...
    async def headpat(self, ctx):
        ...
        if ctx.message.author.id in config.config.mods:
            await ctx.send(util.get_random_index(config.lines.shutdown))
            await self.bot.change_presence(status=discord.Status.offline)
            await self.bot.logout()
            await self.bot.close()
        else:
            await ctx.send(util.get_random_index(config.lines.dumb))
    ...
    async def prefix(self, ctx, prefix: str = None):
        ...
        with db.database.session() as session:
            ...
            if prefix is None:
                await ctx.send(
                    "The prefix is `{}`".format(user.active_server.prefix)
                )
                return
    ...
    async def getmod(self, ctx):
        ...
        with db.database.session() as session:
            ...
            try:
                ...
                await ctx.send("There is no moderator set")
    ...
    async def setmod(self, ctx, *, role_name: str):
        ...
        with db.database.session() as session:
            server = db.database.getServerFromCtx(
                session, ctx, commit=False
            )[0]
                ...
                if member.server_permissions.manage_server or \
                    ...
                    for role in ctx.message.server.roles:
                        if role_name in [
                            ...
                            message.append(
                                "I made {} a moderator!".format(name)
                            )
