...
def load():
    ...
    def mergeDict(old: dict, new: dict, layer=1) -> dict:
        """
        Merge a dictionary into another while prefering the old values over
        the new
        ...
        for key, val in new.items():
            ...
            if key not in old:
                _logger.debug("{}Adding new value {}".format(
                    '  ' * layer, key))
                changed = True
                old[key] = val
            elif issubclass(type(old[key]), Mapping) \
                    and issubclass(type(val), Mapping):
...
def get_defaults():
    ...
    defaults['lines'] = dict(
        crits=[u"Headshot!", u"Critical Hit!", u"Booyeah!", u"Crit!",
               u"Finish him!", u"Get pwn'd!"],
        critFails=[u"Oof", u"Fatality!", u"Ouch, ooch, oof your bones!",
                   u"That'll hurt in the morning..."],
        dumb=[u"...What did you think would happen?", u"...Why?",
              u"Are you ok?",
              u"Do you need a doctor?",
              u"What else did you think it would do?"],
        memes=[u"You.", u"I'm running out of memes...",
        ...
        startup=[u"*Yawn* Hello friends!", u"おはようございます!",
                 u"おはよう、お父さん", u"Ohayō, otōsan!",
                 "Alright, who's ready to die?", u"Greetings humans.",
                 u"My body is Reggie."],
        shutdown=[u"Bye!", u"Farewell comrades!", u"さようなら、お父さん!",
        ...
        user_error=dict(
            no_user=[u"You don't even have a character!",
                     u"You aren't registered in the system."],
            no_char=[u"You don't have any characters!",
                     u"Why would you try to do this when you don't even have a character?"],
            wrong_char=[u"It looks like we have a missing character.  Let's start a search party",
                        u":wave: This isn't the character you are looking for."],
            too_many_char=[u"You have too many characters, specify your character next time.",
...
def migrate(version):
    _logger.info("Migrating old config version from v{} to v{}..".format(
        version, VERSION))
    if version == -1:
